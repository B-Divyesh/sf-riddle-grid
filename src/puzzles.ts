...
const seeds: PuzzleSeed[] = [
  ['Daily deduction grid', 'Read the vertical clues before the horizontal clues.', [0, 2, 1, 3], [1, 3, 0, 2]],
  ['Daily deduction grid', 'Start with the specimen that sits highest.', [1, 3, 0, 2], [2, 0, 3, 1]],
  ['Daily deduction grid', 'A long chain can be solved from either end.', [2, 0, 3, 1], [3, 1, 0, 2]],
  ['Daily deduction grid', 'Place one chain, then cross it with the other.', [3, 1, 2, 0], [0, 2, 1, 3]],
  ['Daily deduction grid', 'Each row and column accepts one specimen.', [0, 3, 2, 1], [2, 1, 3, 0]],
  ['Daily deduction grid', 'The same four clues make two ordered chains.', [2, 1, 0, 3], [1, 0, 2, 3]],
  ['Daily deduction grid', 'Edges are useful anchors even without a border clue.', [1, 0, 3, 2], [3, 2, 0, 1]],
  ['Daily deduction grid', 'Follow every arrow before checking the grid.', [3, 2, 1, 0], [1, 3, 2, 0]],
  ['Daily deduction grid', 'One specimen never shares a row or column.', [0, 1, 3, 2], [3, 0, 2, 1]],
  ['Daily deduction grid', 'The two clue chains cross only once per specimen.', [2, 3, 1, 0], [0, 1, 3, 2]],
  ['Daily deduction grid', 'Use the field rules to turn order into exact cells.', [1, 2, 0, 3], [2, 3, 1, 0]],
  ['Daily deduction grid', 'Top and left are the starts of separate chains.', [3, 0, 1, 2], [1, 2, 3, 0]],
  ['Daily deduction grid', 'A complete chain fixes all four positions.', [0, 2, 3, 1], [2, 0, 1, 3]],
  ['Daily deduction grid', 'Read relationships, not the order of the cards.', [2, 0, 1, 3], [1, 3, 0, 2]],
  ['Daily deduction grid', 'Move a placed specimen again if a later clue disagrees.', [1, 3, 2, 0], [0, 2, 3, 1]],
  ['Daily deduction grid', 'Rows run top to bottom. Columns run left to right.', [3, 1, 0, 2], [2, 0, 1, 3]],
  ['Daily deduction grid', 'Every clue belongs to both a row and a column chain.', [0, 3, 1, 2], [1, 2, 0, 3]],
  ['Daily deduction grid', 'An empty cell can help you see the remaining column.', [2, 1, 3, 0], [3, 0, 2, 1]],
  ['Daily deduction grid', 'Hints show exact cells but reduce the leaf score.', [1, 0, 2, 3], [2, 3, 0, 1]],
  ['Daily deduction grid', 'Check only after every specimen is on the field.', [3, 2, 0, 1], [0, 1, 3, 2]],
];
